...
def get_subfolder_paths(directory):
    ...
    for folder_name in os.listdir(directory):
        ...
        if os.path.isdir(full_path):
            for sub_folder_name in os.listdir(full_path):
                full_subpath = os.path.join(full_path, sub_folder_name)
                if os.path.isdir(full_subpath):
                    if 'saved_samples' in os.listdir(full_subpath):
                        subfolder_paths.append(full_subpath)
    ...
    for folder_name in os.listdir(directory):
        ...
        if os.path.isdir(full_path):
            for sub_folder_name in os.listdir(full_path):
                full_subpath = os.path.join(full_path, sub_folder_name)
                if os.path.isdir(full_subpath):
                    if 'raw_data.hdf5' in os.listdir(full_subpath):
                        subfolder_paths.append(full_subpath)
...
def download_process_copy_files(subfolders, subfolders_new_location, filename="config_mcmc_bayes.yaml", drop_prerejected=False, verbose=True):
    for old_folder, new_folder in zip(subfolders, subfolders_new_location):
        if verbose:
            print("Processing folder: ", old_folder)
        old_file_path = os.path.join(old_folder, 'common_files', filename)
        new_file_path = os.path.join(new_folder, filename)

        # Check if the file exists in the old folder
        if os.path.isfile(old_file_path):
            # Copy the file to the new folder
            shutil.copy2(old_file_path, new_file_path)

        raw_data_loaded = RawData()
        raw_data_loaded.load_from_folder(old_folder)
        if drop_prerejected:
            raw_data_loaded = raw_data_loaded.filter(types=[0, 2])
        raw_data_loaded.save_hdf5(os.path.join(new_folder, "raw_data.hdf5"))
